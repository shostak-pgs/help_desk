...
import {setTicket, setEnums} from './../store/CrUpdReducer';
    ...
    componentDidMount(){
        ...
        if (this.props.match.params.id !== undefined) {
            ...
            CrUpdTicketService.getTicket(this.props.match.params.id).then(data => {
                data === null ? this.props.setTicket({newTicket : []})
                : this.props.setTicket({newTicket : data})
            })
    ...
    render() {
        ...
        return(
            <div>{this.props.isFetching ? <Preloader/> : null}
                <Route path={`/ticketConstructor/${id}`} render = { () => <UpdateTicket enumsMap = {this.props.enumsMap}
                                                                                       id = {id}
...
let mapStateToProps = (state) => {
    return {
        ...
        currentTicket : state.ticketOverviewPage.ticket
    }
}

export default connect(mapStateToProps, {
    setTicket, switchIsFetching, setEnums, 
